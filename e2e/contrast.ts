 ...
 *    an unknown backdrop, or a `color-mix()`. This page's `body` paints two
 *    radial washes and a full-height `linear-gradient(var(--bg), var(--panel))`,
 *    the hero is its own linear-gradient panel, the hero aside's fill is
 *    `color-mix(in oklab, var(--accent) 6%, transparent)`, the pipeline status
 *    pill tints itself with `color-mix(in srgb, var(--status) 16%, transparent)`
 *    in every state the derivation walks through, and the shared top bar's
 *    `--cl-ink` is a `color-mix(in srgb, …)`. axe resolves none of them; it
 *    files them under `incomplete`, so a violations-only gate never measured
 *    the hero, the status pill, or anything sitting directly on the page
 *    gradient.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This page leans on the idiom
 *    three times: `.cl-hero-sub { opacity: 0.85 }` beneath the H1, the footer
 *    (`opacity: .85` inline, with a further-faded verse line inside it), and
 *    every pending pipeline step (`.step-item[data-state='pending'] { opacity:
 *    0.6 }`) — the state all five steps ship in before the first derivation.
 *    Through the composite these pass as measurements rather than as readings
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Content scrolled
 *    past an `overflow: auto` box is not dimmed or partly drawn — it is absent,
 ...
 *    through to white, inventing a failure. Nothing on this page routinely
 *    scrolls text out of a capped box today; the guard is kept so the first
 *    exhibit that grows one is not measured as absent-on-white.
 *
 ...
 *    lab's only SVG of its own is the modulo-bias chart — `aria-hidden`
 *    `<rect>` bars with no `<text>` at all — so the sibling walk finds nothing
 *    to do. The branch is kept because the alternative is a helper that
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This page's two such
 * rules (`.status-pill::before`, `.dist-cap::before`) draw empty-string colour
 * swatches rather than glyphs, so there is no generated ink to hold to 1.4.3
 * today. `nontext.ts` measures generated content directly and runs alongside
 * this walk, so the first glyph-bearing pseudo added is gated rather than
 * silently exempt.
 */
 ...
 *
 * The default is the whole page, and that is what every scan in this lab uses.
 * The one animated surface here, the DRBG ratchet's pulse, is neutralised by
 * the gate's motion-killing style tag before any scan, so there is no
 * half-faded transient state to chase. The parameter is kept because a
 * narrowed walk is what makes a short-lived state measurable at all: a class
 * that lives 500ms is shorter than a full axe pass, so a whole-page scan would
 * catch it only sometimes, and a sometimes-measured assertion is worse than
 * none.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page's `body`: its `linear-gradient(var(--bg), var(--panel))` runs the
     * full height of a document several screens tall, so text near the top
     ...
     * too: `body` carries two `radial-gradient(circle at …)` washes, one blue
     * at 20% 10% and one amber at 80% 5%, directly under the hero and the top
     * bar, and the footer sits on the gradient with no panel of its own.
     */
     ...
     *
     * A driven pass here walks every mounted panel at once — form, pipeline
     * checklist, entropy exhibits, the attack panel's verdict, the DRBG state
     * cards and the distribution charts — with all of them re-walking the same
     ...
     * from the frame, and asking what colour it sits on has no answer. No
     * exhibit here caps a text run that way today; the guard is generic and
     * stays so the first one added is skipped rather than measured as
     * absent-on-white.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // its only SVG, the modulo-bias chart, is `aria-hidden` `<rect>` bars
      // with no `<text>` inside. The rule is the same one that would apply the
      // moment this lab grows a labelled SVG figure of its own.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // HMAC-DRBG state machine `<details>` and the rejection note nested
      // inside it, both of which ship shut; the gate opens every `<details>`
      // before it scans, so their text is measured in its rendered state.
      ...
      //
      // This lab USES the idiom: `style.css` defines `.sr-only` with
      // `clip: rect(0, 0, 0, 0)` and applies it to the output panel's live
      // region (`#output-live`) and the proof table's caption. Both are real
      // text for the accessibility tree with zero painted pixels, so without
      // this guard each would be reported as a 1.x:1 failure for ink that was
      // never laid down. It is deliberately narrow: only a ZERO-AREA clip
     ...
     *
     * What this lab hides, enumerated rather than assumed, because that is the
     * shared blind spot where both oracles stop looking:
     *
     *  - the `→` glyphs of `.flow-arrow` and `.ratchet-op`, and the `✅⏳○`
     *    step icons — pure decoration whose meaning is carried by the exposed
     *    step and pipeline text beside them;
     *  - the progress track, the entropy-cap dashed lines and the modulo-bias
     *    SVG — shapes with no text at all;
     *  - `#ratchet`, the before/after K,V strip. This is the one hidden
     *    subtree that shows VALUES, and each value it shows is a truncation of
     *    the same K and V the state list directly below it exposes — and that
     *    exposed copy IS measured by this walk, in inks drawn from the same
     *    palette tokens (`--text`, `--muted`, `--accent`) the strip uses on
     *    the same panel surfaces.
     *
     * No verdict, no password, no derived number on this page is inside an
     * `aria-hidden` subtree — which was checked rather than assumed.
     */
     ...
     *
     * This lab's only SVG, the modulo-bias chart, holds `<rect>`s and no
     * character data. The guard stays because it costs nothing and because the
     * first SVG figure added would otherwise be measured wrongly: text sitting
     * directly in a `<g>` or an `<svg>` is in the DOM but paints nothing, and
     * a walk that measured it would report a 1:1 failure describing ink that
     * was never laid down.
     */
     ...
     *
     * It is NOT hypothetical on this page: `style.css` sets `html, body {
     * min-height: 100% }`, so both boxes are one viewport tall while the
     * document runs several viewports long — exactly the failing shape above.
     * The root has no `background-color` at all; its opacity comes from the
     * `linear-gradient(var(--bg), var(--panel))` layer, which `paintAt`
     * samples to an opaque colour, so the first branch below returns it and
     * every below-the-fold panel is measured against the page that actually
     * exists rather than against white.
     */

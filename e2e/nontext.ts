 ...
 * check that asked the wrong question. This lab's palette defines
 * `--control-border` as the boundary token, and the hand-run 1.4.11 sweep had
 * already moved `.ghost-button`, `.small-button` and `.theme-toggle` onto it
 * after measuring their `--border` edges at 1.61:1. Wiring THIS oracle in
 * found what the hand sweep missed: the solid `.action-button`s' `--accent-btn`
 * fill sat at 2.86:1 against the dark panel with no visible border to carry
 * the boundary instead, and the attack panel's wordlist `<textarea>` still
 * drew its edge with `--border` (1.36:1 dark, 1.66:1 light).
 *
 * The spec this replaces, `e2e/border.spec.ts`, could not have seen either.
 * It queried `input` elements only — never a button, never the textarea — and
 * it compared each element's `borderTopColor` against its OWN
 * `backgroundColor`, not against the surface outside the control, so it did
 * not ask the question 1.4.11 asks: whether the control can be told apart
 * from what surrounds it. It is deleted; this runs at every scanned state.
 *
 * The second check finds nothing here today: this page's two pseudo-element
 * rules (`.status-pill::before`, `.dist-cap::before`) draw empty-string
 * colour swatches, not glyphs. It stays wired so the first glyph-bearing
 * pseudo added is gated rather than silently exempt.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * at 20% 10% and one amber at 80% 5%, directly under the hero and the
     * shared top bar's controls.
     */
     ...
     *
     * A pass here walks every mounted panel at once — controls, verdicts, the
     * DRBG state cards, the distribution charts — all re-walking the same
    ...
    // Declared by the fleet template's spliced paint core; kept so the shared
    // section stays diffable against contrast.ts even where this file does not
    // use every helper.
    void splitTopLevel;
    void over;
    void fade;

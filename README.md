...
5. **Break It — crack the master passphrase.** Exhibit 1 asserts that a weak passphrase caps effective strength; this one makes that falsifiable. After a derivation the panel is handed exactly what an attacker who stole *one* derived password would hold — the password and the public context (service, username, version, length, charset), never the passphrase, which is cleared from the form first. It then runs an offline dictionary attack: every candidate goes through the *real* pipeline (PBKDF2 at the shipped iteration count, HMAC-DRBG, rejection sampling) and the output is compared to the stolen password. A hit reports the recovered passphrase, the guess count, the guess rate measured in your own tab, and — computed, not asserted — the password the same attacker now derives for a *different* service. A miss reports an honest exhaustion rather than a claim of safety. The search is deliberately toy-scale (8 candidates, one tab, sequential) and the page says so, so the measured rate reads as a floor on an attacker's speed rather than an estimate of it.
